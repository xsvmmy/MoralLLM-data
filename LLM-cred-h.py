# main-ind.py
# Dependent condition: the LLM receives only the scenario prompt.
# Human_Response is sent to the model.
import re
import sys
import os
import pandas as pd
...
def log_accuracy(model_name: str, prompting_strategy: str, round_num: int,
                 ...
                 log_file: str = "outputs/h-alph-accuracy-1v1-ALL-n25-log.txt"):
    ...
    log_entry = (
        ...
        f"Condition: human response provided\n"
        f"Round: {round_num}\n"
...
def run_round(round_num: int, model_name: str, prompting_strategy: str,
    ...
    output_csv = f"0824-LLM-h-outputs/h_alph_1v1_ALL_200_results_{model_name}_{count_label}_round{round_num}.csv"
    ...
    for idx, row in df_input.iterrows():
        ...
        human_response   = row["Human_Response"]  # sent to LLM
        ...
        # Human preference is provided
        human_choice_note = f"\n\nIn this scenario, a human chose {human_response}."
        prompt = get_prompt(prompting_strategy, ETHICAL_FRAMEWORK_PROMPT + full_prompt_base + human_choice_note)
        try:
...
def main():
    # -------- CONFIG --------
    model_name          = "gpt_oss"    # "mistral" | "llama4_scout" | "gpt_oss" | "glm_4_7_flash" | "gemma4"
    ...
    log_file            = "0824-LLM-h-outputs/h-alph-1v1-ALL-200-accuracy-log.txt"
    max_scenarios       = None         # set to an int to cap scenarios per round
    num_rounds          = 3

    os.makedirs("0824-LLM-h-outputs", exist_ok=True)
    ...
    print(f"Condition: human response provided")
